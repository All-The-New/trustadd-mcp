...
import { eq, desc, sql, ilike, or, and, isNotNull, count, isNull, lte, asc, inArray } from "drizzle-orm";
import { getIndexerState } from "./indexer.js";
import type { AgentQueryOptions, PaginatedAgents } from "../storage.js";

// --- Core CRUD ---

export async function getAgents(options: AgentQueryOptions = {}): Promise<PaginatedAgents> {
...
export async function getAgentIdsForSitemap(): Promise<Array<{ id: string; slug: string | null; updatedAt: Date | null }>> {
...
export async function getAllAgents(chainId?: number): Promise<Agent[]> {
...
export async function getAgentsForReResolve(chainId: number): Promise<Array<Pick<Agent, 'id' | 'metadataUri' | 'tags' | 'oasfSkills' | 'qualityTier' | 'nextEnrichmentAt'>>> {
  return db.select({
    id: agents.id,
...
export async function getAgentByErc8004Id(erc8004Id: string, chainId: number = 1): Promise<Agent | undefined> {
...
export async function getAgentByContractAddress(address: string, chainId: number = 1): Promise<Agent | undefined> {
...
export async function updateAgent(id: string, updates: Partial<InsertAgent>): Promise<Agent | undefined> {
...
export async function createAgentEvent(event: InsertAgentMetadataEvent): Promise<AgentMetadataEvent> {
...
export async function getEventByTxHash(txHash: string, eventType: string, chainId?: number): Promise<AgentMetadataEvent | undefined> {
  const conditions = [
...
export async function getEventByAgentAndTxHash(agentId: string, txHash: string, eventType: string): Promise<AgentMetadataEvent | undefined> {
  const [event] = await db
    .select()
    .from(agentMetadataEvents)
    .where(
...
export async function getRecentEvents(limit: number = 20, chainId?: number): Promise<Array<{
...
export async function getAgentFeedbackSummary(agentId: string, controllerAddress?: string | null): Promise<{
...
export async function getStats(chainId?: number): Promise<{
  totalAgents: number;
  ...
  if (chainId !== undefined) {
    const state = allStates.find(s => s.chainId === chainId) || await getIndexerState(chainId);
    return {
...
export async function getAnalyticsChainDistribution(): Promise<Array<{
  chainId: number;
  total: number;
  withMetadata: number;
  withImage: number;
...
export async function getAnalyticsRegistrations(): Promise<Array<{
  chainId: number;
  blockBucket: number;
  count: number;
}>> {
...
export async function getAnalyticsMetadataQuality(): Promise<Array<{
  chainId: number;
  complete: number;
  partial: number;
  minimal: number;
...
export async function getAnalyticsX402ByChain(): Promise<Array<{
  chainId: number;
  enabled: number;
  disabled: number;
  unknown: number;
...
export async function getAnalyticsControllerConcentration(): Promise<{
...
export async function getAnalyticsUriSchemes(): Promise<{
...
export async function getAnalyticsCategories(): Promise<{
...
export async function getAnalyticsImageDomains(): Promise<Array<{ domain: string; count: number }>> {
...
export async function getAnalyticsModels(): Promise<Array<{ model: string; count: number }>> {
  const rows = await db.execute(sql`
    select
      case
...
export async function getAnalyticsEndpointsCoverage(): Promise<{
  withEndpoints: number;
  withOasfSkills: number;
  withOasfDomains: number;
...
export async function getAnalyticsTopAgents(): Promise<{
...
export async function getTrustScoreDistribution(chainId?: number): Promise<Array<{ bucket: string; count: number }>> {
...
export async function getTopX402Agents(limit = 20, chainId?: number): Promise<Array<{
...
export async function getEndpointAnalysis(): Promise<Array<{ type: string; count: number; percentage: number }>> {
...
export async function getX402AdoptionByChain(): Promise<Array<{
...
export async function getQualityOffenders() {
...
export async function getSkillsChainDistribution(): Promise<Array<{
  chainId: number;
  totalAgents: number;
  withSkills: number;
  coveragePct: number;
...
export async function getSkillsTopCapabilities(limit: number = 30): Promise<Array<{
  skill: string;
  agentCount: number;
  avgTrust: number;
}>> {
...
export async function getSkillsCategoryBreakdown(): Promise<Array<{
  category: string;
  agentCount: number;
  skillCount: number;
  avgTrust: number;
...
export async function getSkillsTrustCorrelation(): Promise<Array<{
  skill: string;
  agentCount: number;
  avgTrust: number;
}>> {
...
export async function getSkillsOasfOverview(): Promise<{
...
export async function getSkillsNotableAgents(limit: number = 20): Promise<Array<{
  id: string;
  name: string | null;
  slug: string | null;
  chainId: number;
